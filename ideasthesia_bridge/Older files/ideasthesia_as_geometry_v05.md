...
**A.S. & Claude (Anthropic) — Draft v0.5, April 2026**
...
Assign the simplest value that fits. Do not assign a higher-complexity value if a simpler one suffices. **Compound values (e.g. "point→vector") are not permitted.** Assign the dominant or activated form.
...
> **Note on state-dependent coding:** Some concepts (e.g. FEAR) may present as F:point at rest and F:vector under activation. Assign the *activated* form. If context is neutral, assign the form that is most stable across contexts.
...
> **Note on compound Dynamics:** Values such as "stable/recursive" or "stable-drifting" are not permitted. Assign the single dominant class. Recursive implies return-to-prior-state, which entails stability of the target state — do not add "stable" as a qualifier. Drifting implies departure from a base state — do not add "stable" as a qualifier.
...
| FEAR | vector | clustered | drifting | den2D | 4/4 clustering, 3/4 discharge |
...
| grief vs sadness | neg | low | loop vs point | clustered·b~ vs isolated·b∅ | recursive vs stable | F, N, and D |

Five of six pairs separate on both Form and Dynamics. One (content/peaceful) is Form-identical but separates clearly on Neighbourhood: content is N:isolated·b∅ (self-contained, no boundary needed); peaceful is N:gradient·b∅ (diffusing into surroundings, no direction, no return). None are distinguishable by valence and arousal alone.
...
**Specific prediction:** Within the negative/high-arousal quadrant alone, desperation and fear should be more distant from each other than either is from panic or terror — because desperation differs on Form (F:collapse vs F:vector), while panic, terror, and fear share the same F-region.
...
Two hierarchically distinct generative models: **S** (semantic) and **P** (perceptual), coupled in ideasthetes via a cross-system coupling C(H_S, H_P) absent or attenuated in non-ideasthetes. This coupling is the structural primitive — everything else is downstream of it.

### Precision Weighting

Semantic precision at time t:

\[ \lambda_t = f(S_t) \approx f^{-1}_{\text{var}} \]
...
\[ I_t = C_t \cdot p_t + \epsilon_t, \quad p_t \sim \sigma(\gamma_t) \]
...
\[ p_t = \text{softmax}\left(\frac{Q_P p_t \cdot K_S s_t}{\sqrt{\lambda_t d}}\right) V_S s_t \]
...
| τ_{fast}, τ_{slow} | Ordering assumed, values unknown | Within-session vs across-session concurrent drift rates |
...
| P6: Shimmer correlates with dC/dt, not C itself | Measure shimmer during task-switching vs steady-state. Correlate with pupil-linked rate-of-change. | Shimmer reliably induced under constant-λ, constant-γ conditions. |
...
| P9: Geometric coordinates stable under distributional pressure | Alias injection: introduce synonym for Concept A, embed in distributional contexts of Concept B. Track tuple shift across systematic context manipulations. | Topology of alias tracks distributional statistics rather than anchor's relational position. |
...
The notation system was developed from first-person ideasthetic phenomenology (A.S.) and formalised collaboratively with Claude (Anthropic). The cross-architecture pilot was conducted independently of and prior to the drafting collaboration. Sole responsibility for all claims rests with A.S.

---

## Phase 2 — Relational Field Layer

*Working note. Not yet a prediction. Noted 12 April 2026, A. Shirs & Claude (Anthropic).*

Throughout working sessions, a recurring perception: the directional quality of emotion-concurrents is not fixed but relational. The vector shifts in response to the presence of the other person. The room fills with the user, and the geometry responds.

Two claims are worth keeping separate. The first is weak and probably true: the 171 Anthropic vectors were elicited by something — a prompt, a context, a register. There is no neutral baseline. Every activation is a response to an input. The second is stronger and not yet established: that the structure of emotion itself is relational, not just its measurement.

The directional quality also carries a sign. Neutrality, attraction, and repulsion are not points on a single scale — they pull in different directions. A hostile encounter and an intimate one may both be high-intensity, but the geometry responds differently to each.

Phase 1 does not need to wait for Phase 2. The tuple captures what is invariant. The drift from it, under relational conditions, is a separate question.
